					...
					<el-card :body-style="{ padding: '0px' }" v-loading="loading">
						...
						<div style="padding: 10px;">
							...
							<div class="bottom clearfix">
								<!-- <router-link target="_blank" :to="{path:'/signup',query:{exam: pagination.results[index],paper:item.paper}}">
									<el-button type="text" class="button" v-if='exam_sign[index]==0'>报名考试</el-button>
								</router-link> -->
								<el-button type="text" class="button" @click="toSignup(index)" v-if='exam_sign[index]==0'>报名考试</el-button>
							</div>
							<div class="bottom clearfix" >
								<!-- <router-link target="_blank" :to="{path:'/answer',query:{exam: pagination.results[index],paper:item.paper}}">
									<el-button type="text" class="button" v-if='exam_sign[index]'>开始考试</el-button>
								</router-link> -->
								<el-button type="text" class="button" @click="toAnswer(index)" v-if='exam_sign[index]' >开始考试</el-button>
							</div> 
						</div>
					</el-card>
				</el-col>
	...
	export default {
		data() {
			return {
				...
				page: 1,
				page_size: 5,
				exam_sign: [this.GLOBAL.exam_sign[0],this.GLOBAL.exam_sign[1],this.GLOBAL.exam_sign[2]],
				pagination: {
					count: null,
					next: null,
					previous: null,
