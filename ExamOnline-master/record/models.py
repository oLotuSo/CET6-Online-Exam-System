...
class FillRecord(Record):
    """翻译题答题记录"""
    fill = models.ForeignKey(Fill, verbose_name="翻译题", on_delete=models.CASCADE)

    class Meta:
        ordering = ['id']
        verbose_name = '翻译题答题记录'
        verbose_name_plural = verbose_name
...
class ProgramRecord(Record):
    program = models.ForeignKey(Program, verbose_name="作文题", on_delete=models.CASCADE)
    cmd_msg = models.TextField("输出结果", null=True, blank=True)

    class Meta:
        ordering = ['id']
        verbose_name = '作文题答题记录'
        verbose_name_plural = verbose_name
